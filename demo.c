...
void run_debugger(pid_t child_pid)
{
	...
	if(task_port){
		err = task_threads(task_port,&threads,&num_threads);
		if(err)
			printf("error: task_threads\n");
	printf("num_threads=%d\n",num_threads);
	for(i=0;i<num_threads;i++){
		printf("new thread [%d]\n",threads[i]);
		err = thread_set_exception_port(threads[i],port);
		if(err)
			printf("error: thread_set_exception_port\n");
		err = mach_port_request_notification(mach_task_self(), threads[i], MACH_NOTIFY_DEAD_NAME, 1, port, MACH_MSG_TYPE_MAKE_SEND_ONCE,&prev_port);
		if(err)
			printf("error: thread_set_exception_port\n");
		else {
			if(prev_port != MACH_PORT_NULL)
...
#if 0
		printf("thread_get_state from thread [%d]\n",threads[i]);
		err = thread_get_state(threads[i], THREAD_STATE_FLAVOR, (thread_state_t)&state, &state_size);
		if(err != KERN_SUCCESS)
			printf("error: thread_get_state return %d\n",err);
		else
			printf("thread_get_state state_size=%d\n",state_size);
#endif
		}
	}
	err = proc_wait_request(getproc(), port,child_pid,2);
	...
	err = task_threads(task_port,&threads2,&num_threads);
	if(err)
		printf("error: task_threads2\n");
	printf("num_threads=%d\n",num_threads);
	for(i=0;i<num_threads;i++){
		printf("new thread [%d]\n",threads2[i]);
		err = thread_set_exception_port(threads2[i],port);
		if(err)
			printf("error: thread_set_exception_port\n");
		err = mach_port_request_notification(mach_task_self(), threads2[i], MACH_NOTIFY_DEAD_NAME, 1, port, MACH_MSG_TYPE_MAKE_SEND_ONCE,&prev_port);
		if(err)
			printf("error: thread_set_exception_port\n");
		else {
			if(prev_port != MACH_PORT_NULL)
